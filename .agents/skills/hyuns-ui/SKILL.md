---
name: hyuns-ui
description: Use when building, modifying, reviewing, or documenting Hyuns UI usage inside the hyuns-sites workspace, including @hyunsdev/ui npm consumption, @workspace/site-ui facade exports, app-level React/Tailwind v4 styling, portal.yml-driven navigation, web-color UI integration, and validation of apps that consume Hyuns UI.
---
...
2. Read the target app or package manifest before editing: `apps/portal/package.json`, `apps/web-color/package.json`, `apps/example/package.json`, or `packages/site-ui/package.json`.
...
- Local facade: `@workspace/site-ui` currently re-exports only `Button` and `buttonVariants` from `@hyunsdev/ui`.
...
- This checkout has no `packages/ui`, no `packages/playground-ui`, and no `playground/ui-playground`. Those belong to the separate `hyuns-ui` repo, not this workspace.
...
1. Identify the target surface: Portal, Web Color, example app, `@workspace/site-ui`, shared config, style entry, route registration, or browser-visible UI behavior.
...
- Do not ask for or read `packages/ui/SKILL.md`, `packages/ui/package.json`, `packages/playground-ui`, or `playground/ui-playground` in this repo.
